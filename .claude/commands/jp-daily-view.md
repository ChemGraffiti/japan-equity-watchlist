...
4.5. **資金フロー分析 / Capital flow analysis**
   ...
   - **正午実行時は米国・中国・韓国のこの節を省略する**（US/CN/KRのデータは朝から更新していないため）。代わりに「米国・中国・韓国の資金フロー分析は本日朝の更新分を参照（`daily/{date}.md`）」と一言添えるのみでよい。
...
4.6. **本日の注目銘柄トップ3 / Today's Top 3 stocks to watch**
   ...
   - **正午実行時は米国・中国・韓国のトップ3選定を省略する**（US/CN/KRのデータは朝から更新していないため、当日の値動きに基づく選定ができない）。日本のトップ3のみでよい。
