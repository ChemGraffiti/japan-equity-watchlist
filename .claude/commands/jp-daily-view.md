...
**同時に起動する調査用サブエージェントは合計8〜10個までとし、1エージェントあたりの担当ティッカー数を約30〜50件に収めること。** WebSearchはセッション全体（メイン＋全サブエージェント）で予算(200回程度)を共有している。過去2回の失敗例:
- 2026-07-27 10:30の1回目は16並列で起動しセッション予算を数分で枯渇させ失敗（`daily/2026-07-27_正午.md`に失敗報告あり）。
- 直後に「並列4〜6個までに統合」で再実行したところ、**JP+US+CN+KRを同じバッチに統合した結果、1エージェントあたりの調査量が単純合算で4倍(最大110件超)に膨れ上がり**、3時間半経過しても応答・コミットなしでハングした（該当セッション: `cse_01UypNK9zvTHSeKY7wN1rsU4`、2026-07-27 10:30起動）。
- **教訓**: 並列数を絞ること自体は正しいが、「バッチを統合して並列数だけ減らす」のは1エージェントの負荷を増やすだけで逆効果。**市場ごとに別バッチにする**（JP用に4バッチ、US/CN/KR用にそれぞれ1〜2バッチ、合計8〜10バッチ）ことで、並列数と1エージェントあたりの負荷の両方を適正化する。
