...
- **正午実行時はUS_PRICEの調査を省略する**（正午時点で米国市場は未開場のため、朝と実質同じ値になり調査が無駄になる。ステップ2参照）。
...
2. **ダッシュボードの株価を更新 / Refresh the dashboard's DAILY object**
   ...
   - **正午前後（11:00〜14:00頃）に実行する場合は、`US_PRICE`の調査・更新を省略してよい**（米国市場が未開場のため朝の値と実質同じになり調査が無駄になる。`USBASE`・`US_PRICE`は前回の値のまま据え置く）。この場合はJP4バッチ＋CN/KR用バッチのみでよい。
